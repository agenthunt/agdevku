...
int BPlusTreeUtil::keyCompare(const char* key1, const char* key2,
	...
	for (int i = 0; i < numOfColumns; ++i) {
		int colType = indexHeader->columnTypes_[i];
		switch (colType) {
		case COL_INTEGER:
			//			cout << "\nINTEGER TYPE" << endl;
		...
		offset = offset + indexHeader->columnSizes_[i];
	}
	return 0;
...
void BPlusTreeUtil::displayKey(const char *key,
	...
	for (int i = 0; i < numOfColumns; i++) {
		...
		case COL_LONG:
			...
			break;
		case COL_VARCHAR:
			memcpy(varCharKey, &key[offset], indexHeaderPage_->columnSizes_[i]);
			cout << varCharKey;
			break;
		default:
			memcpy(varCharKey1, &key[offset], indexHeaderPage_->columnSizes_[i]);
			cout << varCharKey1;
			break;
		}
...
std::string BPlusTreeUtil::KeytoString(const char* key,IndexHeaderPage *indexHeaderPage_)
{
